      ...
      <div class="col-md-10 col-lg-10 col-xl-8">
        <div class="card mb-3 m-auto ">
          ...
          <div class="card-header d-flex ">
            <div class="d-flex flex-fill justify-content-start align-items-center">
              ...
              <div>
                <span><i v-if="current_user_email === this.$props.email_owner_post || current_user_admin == 'true'"
                    class="fa-solid fa-pen-to-square ps-2" data-bs-toggle="tooltip" title="Mise à jour du profile"
                    @click="UpdateProfile"></i>
                </span><span><i v-if="current_user_admin == 'true'" class="fa-solid fa-user-slash ps-2"
                    data-bs-toggle="tooltip" title="Suppression du profile !" @click="DeleteProfile"></i></span>
              </div>
            </div>
            ...
            <div class="d-flex flex-fill justify-content-end">
              <span><i v-if="current_user_email === this.$props.email_owner_post || current_user_admin == 'true'"
                  class="fa-solid fa-trash pe-2" data-bs-toggle="tooltip" title="Suppression du post!"
                  @click="deletePost"></i>
              </span>
              <span><i v-if="current_user_email === this.$props.email_owner_post || current_user_admin == 'true'"
                  class="fa-solid fa-pen-to-square pe-2" data-bs-toggle="tooltip" title="Mode édition"
                  @click="editPost"></i>
                <i v-if="modeEditPost && url_img_owner_post && !this.post_img_hidden"
                  class="fa-solid fa-file-circle-minus pe-2" data-bs-toggle="tooltip" title="Suppression image"
                  @click="hidePostImage"></i>
                <label for="this.post_id"><i v-if="modeEditPost" class="fa-solid fa-file-circle-plus pe-2"
                    data-bs-toggle="tooltip" title="Nouvelle image"></i></label>
                <i v-if="modeEditPost" class="fa-solid fa-floppy-disk" data-bs-toggle="tooltip"
                  title="Sauvegarde des modifications" @click="updatePost"></i></span>
            </div>
          </div>

          <p class="pt-2 fs-6 text-muted ">{{ createdAt }}</p>

          <img id="image_post" v-if="url_img_owner_post && !this.post_img_hidden" :src="url_img_owner_post"
            class="card-img-top py-2" alt="image du post" />
          ...
          <div class="card-body">
            ...
            <div v-if="modeEditPost" class="d-flex flex-start align-items-center border">
              ...
              <div class="image-input m-auto" :style="{ 'background-image': `url(${imagePreview})` }"></div>
              <input id="this.post_id" class="file-input" ref="fileInput" type="file" @input="addPostImage"
                accept="image/*">
            </div>
        ...
        <input  class="form-control py-3"  v-model="contentPostModified"/>
            ...
            <div v-for="comment in comments_owner_post">
              <p></p>
              <Comment :email="comment.user.email" :content="comment.content" :urlAvatar="comment.user.avatar"
                :createdAt="comment.createdAt"></Comment>
              <p></p>
            </div>
            ...
            <div class="d-flex gap-1">
              ...
              <input type="text" class="form-control" placeholder="Un petit commentaire..." aria-label="Username"
                v-model="currentComment" />
              <span><i class="fa-solid fa-paper-plane" data-bs-toggle="tooltip" title="Envoi du commentaire"
                  @click="addComent"></i></span>
